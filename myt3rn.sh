...
function execute_script() {
    # 下载文件
    if [ -f "executor-linux-v0.21.11.tar.gz" ]; then
        echo "文件 executor-linux-v0.21.11.tar.gz 已存在，跳过下载。"
    else
        ...
        # 解压文件到当前目录
        echo "正在解压文件..."
        tar -xvzf executor-linux-v0.22.0.tar.gz
    fi
    ...
    # 删除压缩文件
    echo "删除压缩包..."
    rm executor-linux-v0.22.0.tar.gz
